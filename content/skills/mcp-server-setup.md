...
steps:
  - title: Initialize Project
    description: Set up the initial project structure for your MCP server.
    action: >-
      Run the following commands in your terminal: `mkdir my-weather-mcp && cd
      my-weather-mcp && npm init -y && npm install @modelcontextprotocol/sdk &&
      npm install -D typescript @types/node tsx && npx tsc --init`.
    codeSnippet: >-
      mkdir my-weather-mcp && cd my-weather-mcp && npm init -y && npm install
      @modelcontextprotocol/sdk && npm install -D typescript @types/node tsx &&
      npx tsc --init
    expectedResult: >-
      A new directory named 'my-weather-mcp' is created with a package.json file
      and TypeScript initialized.
    failureHint: >-
      Ensure you have Node.js installed and that your terminal is open in the
      desired directory.
  - title: Create Server Code
    description: Implement the server functionality in TypeScript.
    action: >-
      Create a file named `src/index.ts` and copy the provided server code into
      it.
    codeSnippet: >-
      import { Server } from "@modelcontextprotocol/sdk/server/index.js"; ... //
      full server code here
    expectedResult: >-
      The `src/index.ts` file contains the complete server implementation
      without errors.
    failureHint: Check for any syntax errors or missing imports in your TypeScript code.
  - title: Run the MCP Server
    description: Start the MCP server to listen for requests.
    action: Execute the command `tsx src/index.ts` in your terminal.
    codeSnippet: tsx src/index.ts
    expectedResult: >-
      The server starts successfully, displaying a message indicating it is
      listening for requests.
    failureHint: >-
      Ensure TypeScript is correctly compiled and that there are no runtime
      errors in the code.
  - title: Connect to Claude Desktop
    description: Modify the Claude Desktop configuration to connect to your MCP server.
    action: >-
      Edit the Claude Desktop configuration file located at
      `~/Library/Application Support/Claude/claude_desktop_config.json` or
      `%APPDATA%\Claude\claude_desktop_config.json` to include your MCP server
      details.
    expectedResult: >-
      The configuration file is updated successfully, allowing Claude Desktop to
      connect to your MCP server.
    failureHint: >-
      Check the file path for your operating system and ensure the JSON
      structure is valid.
---
