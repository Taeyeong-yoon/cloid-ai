...
- **❌ 사고가 과하다고 판단해 아예 끈다** — 끄는 것보다 effort를 낮추는 편이 안전합니다. 사고를 끄면 오히려 답변이 장황해지는 경우가 있습니다.
- **❌ `xhigh`·`max`를 쓰면서 `max_tokens`를 그대로 둔다** — 사고가 예산을 먹어 답변이 중간에 잘립니다. 높은 effort에는 넉넉한 출력 예산이 필요합니다.
- **❌ 프롬프트로 "생각하지 마"라고 지시한다** — 설정으로 조절하세요. 프롬프트로 억제하면 결과가 불안정해집니다.

## 스스로 점검하기

**Q1. `budget_tokens`를 대체하는 것은?**

정답: `thinking: { type: "adaptive" }` + `output_config.effort`입니다.

**Q2. 스트리밍 UI에서 응답 전 정적이 길게 느껴진다면?**

정답: 사고 표시가 기본 비공개이기 때문입니다. `display: "summarized"`를 켜세요.
